...
class MainWindow(ctk.CTk):
    ...
    def dynamic_geometry(self):
                ...
                while self.background_flag:
                    time.sleep(0.01)
                    if (current_geometry != self.geometry() 
                        or self.content_frame.winfo_width() != round(self.winfo_width()*(1340/1920), 0) and not self.filter_flag 
                        or self.content_frame.winfo_width() != round(self.winfo_width()*(1200/1920), 0) and self.filter_flag):

                        print(self.content_frame.winfo_width(), int(self.winfo_width()*(1340/1920)))
                        if self.filter_flag:
                            ...
                            elif self.apply_button.grid_info()['row'] == self.switch.grid_info()['row']+7:
                                    self.apply_button.grid_forget()
                            ...
                            new_height = self.winfo_height()
                            try:
                                self.drone_frame.configure(width=new_width*(1200/1920))
                                for image in self.images_tab.image_list:
                                    image.configure(size=((int(new_width*(500/1920))), int(new_height*(500/1080))))
                            except Exception as e:
                                print(e)
                            self.menu_frame.configure(width=new_width*(300/1920), height=new_height*(900/1080))
    ...
    def cancel_filter(self):
        ...
        self.filter_flag = False
        self.content_frame.configure(width=1200, height=900)
    ...
    def open_drone(self, id):
        for drone in test_dict:
            if drone['id'] == id:
                ...
                self.drone_frame.grid_rowconfigure((2,3), weight=1)
                self.drone_frame.grid_columnconfigure(1, weight=1)
...
class DroneDescription:
    def __init__(self, master, name, type, characteristics:dict, description):
        ...
        self.name_label.grid(column=1, row=1, sticky='nw', pady=(10, 0))
        
        self.widget_list.append(self.name_label)
        self.type_label = ctk.CTkLabel(master, text=type, font=('Arial', 50, 'bold'))
        self.type_label.grid(column=1, row=2, sticky='nw', pady=(0, 100))
        self.widget_list.append(self.type_label)

        self.char_title_label = ctk.CTkLabel(master, text='Характеристики', font=('Arial', 30, 'bold'))
        self.char_title_label.grid(column=1, row=3, sticky='nw', pady=(10, 0))
